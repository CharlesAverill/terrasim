...
let create_program glsl_v =
  compile_shader (vertex_shader glsl_v) Gl.vertex_shader
  >>= fun vid ->
  compile_shader (fragment_shader glsl_v) Gl.fragment_shader
  >>= fun fid ->
...
let tri ~gl:((maj, min) as gl) win =
  Sdl.init Sdl.Init.video
  >>= fun () ->
  (* create_window ~gl                >>= fun (win, ctx) -> *)
  create_geometry ()
  >>= fun (gid, bids) ->
  create_program (glsl_version gl)
  >>= fun pid ->
  event_loop win (draw pid gid)
  >>= fun () ->
  delete_program pid
  >>= fun () ->
  delete_geometry gid bids
  >>= fun () ->
  (* destroy_window win ctx           >>= fun () -> *)
  Sdl.quit () ; Ok ()

let main win =
  match tri ~gl:(4, 0) win with
  | Ok () ->
      exit 0
  | Error (`Msg msg) ->
      Sdl.log "%s@." msg ; exit 1
